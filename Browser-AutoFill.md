...
But first, we would like to warn you about [criticism of KeePassHTTP security](https://github.com/pfn/keepasshttp/issues/258). That's why this extension not enabled by default in KeeWeb. [Auto-Type](https://github.com/keeweb/keeweb/wiki/Auto-Type) can be a solid alternative, moreover, it's working with other apps, not only browsers. It's recommended to try auto-type approach first.

## KeeWebHttp

Auto-type is possible only in Desktop apps, it's not available in the web version.
